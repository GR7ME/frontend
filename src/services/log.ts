...
export const getFilteredLogs = async ({ period, security_info, logGroupName, logStreamName }: getFilteredLogsInput) => {
    ...
    try {
        const response = await api.get(`/cloudwatch/filter-logs/?${params.toString()}`);
